...
class BigQueryService:
    ...
    def get_dashboard(self, dashboard_id: str, dataset_id: str = "analytics_dash") -> Optional[Dict[str, Any]]:
        table = self.ensure_dashboards_table(dataset_id)
        sql = f"SELECT id, name, version, kpis, layout, layouts, selected_tables, global_filters, theme, CAST(created_at AS STRING) AS created_at, CAST(updated_at AS STRING) AS updated_at FROM `{table}`